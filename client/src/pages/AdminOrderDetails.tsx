...
export default function AdminOrderDetails() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data = (await res.json()) as Order & { message?: string };
  ...
  async function changeStatus(nextStatus: OrderStatus) {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : "Неуспешна промяна на статуса");
    } finally {
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div style={{ opacity: 0.8 }}>
                <div><b>ID:</b> <span style={{ fontFamily: "monospace" }}>{order.id}</span></div>
            ...
            <div style={{ display: "grid", gap: 6 }}>
              ...
              <div>
                ...
                {order.paymentMethod === "card"
                  ? `💳 Карта — ${order.paymentStatus === "paid" ? "платено" : order.paymentStatus === "failed" ? "неуспешно" : "изчаква"}`
                  : "💵 Наложен платеж"}
              </div>
            </div>
