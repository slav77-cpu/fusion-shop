...
export default function AdminOrders() {
  ...
  async function updateStatus(id: string, nextStatus: OrderStatus) {
    ...
    try {
      const data = await apiSend<{ id: string; status: OrderStatus }>(
        `/orders/${id}/status`,
        "PATCH",
        { status: nextStatus },
        { admin: true }
      ...
      setItems((prev) => prev.map((o) => (o.id === id ? { ...o, status: data.status } : o)));
      ...
      alert(e instanceof Error ? e.message : "Неуспешна промяна на статуса");
    } finally {
      setBusyId(null);
    }
  }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data = (await res.json()) as Paginated<Order> & { message?: string };
                ...
                <tr key={o.id} className="aoTr">
                  ...
                  <td className="aoTd">
                    {o.paymentMethod === "card" ? (
                      <span title={`Статус на плащане: ${o.paymentStatus}`}>
                        💳 {o.paymentStatus === "paid" ? "Платено" : o.paymentStatus === "failed" ? "Неуспешно" : "Изчаква"}
                      </span>
                    ) : (
